...
**混合模式（管理员 / 企业版接真实 DeepSeek）**：同时设置 `VITE_USE_MOCK=1` 与 `VITE_API_BASE_URL=https://你的后端域名`（无尾斜杠）。登录、用户信息、配额及 **管理员或企业版会员的 AI 问答**（含流式）会请求真实后端；其余模块仍为 Mock。后端需在 `APP_CORS_ORIGINS` 中加入前端来源，并在 `.env` 中配置 `AI_API_KEY`（DeepSeek 等）。

### 8. Vercel 部署（连接 GitHub 仓库）

仓库已包含 **`vercel.json`**（根目录）与 **`frontend/vercel.json`**，与 Vercel 连接 **GitHub → 本仓库 `main`** 即可构建。请按下面对齐 **Vercel 控制台**，避免与仓库配置冲突。

| Vercel 设置 | 推荐值 | 说明 |
|-------------|--------|------|
| **Root Directory** | **留空**（仓库根目录） | 使用根目录 `vercel.json`：`install` / `build` / `outputDirectory` / SPA 路由 / 构建期 `VITE_USE_MOCK` 已写死。 |
| **Root Directory**（备选） | **`frontend`** | 仅使用 `frontend/vercel.json`（Vite + 同等 rewrites + Mock）；不要使用「仓库根 + Root=frontend」时又手写一套冲突的 Build 覆盖。 |
| **Framework Preset** | 根目录留空时一般为 **Other**；Root=`frontend` 时为 **Vite** | 若与构建命令重复，以仓库内 `vercel.json` 为准。 |
| **Node.js Version** | **20.x**（或与根目录、**`frontend/.nvmrc`** 一致） | 本仓库在根目录与 `frontend` 均提供 `.nvmrc` 为 `20`。 |
| **Environment Variables** | 静态演示 **可不配** | `VITE_USE_MOCK=1` 已在 `vercel.json` 的 `build.env` 注入；若你在面板里也加了同名变量，请勿设为 `0` 以免与预期不一致。 |
| **混合：真实 DeepSeek 问答** | 增加 `VITE_API_BASE_URL` | 与 Mock 并存时：填后端 HTTPS 根地址（无尾斜杠）。管理员/企业版登录后，AI 问答走该后端调 DeepSeek；**后端 CORS** 须包含本 Vercel 域名。 |

接 **真实后端** 时：删除或改掉两个 `vercel.json` 里 **`build.env.VITE_USE_MOCK`**，改为在 Vercel 配好 API 基地址（需前端支持环境变量），并自行解决跨域与 HTTPS。
